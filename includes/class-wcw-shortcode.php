    ...
    <div class="wpwc-wrap" id="<?php echo esc_attr($uid); ?>" data-initial-slug="<?php echo esc_attr($current); ?>">
      ...
      <?php if ($show_filters): ?>
        ...
        <div class="offcanvas offcanvas-start d-lg-none bg-azzurro-chiaro border-container rounded-end-4 align-self-center" tabindex="-1" id="wpwcOffcanvas"
          ...
          <div class="offcanvas-header position-absolute end-0 top-0">
            <h5 class="offcanvas-title visually-hidden" id="wpwcOffcanvas_label">Filtri attività</h5>
            <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Chiudi"></button>
          </div>
          <div class="offcanvas-body">
            <div class="wpwc-toolbar d-flex flex-wrap justify-content-start gap-2" role="tablist" aria-label="Filtra per attività">
              <?php echo $chips_html; ?>
            </div>
          </div>
        </div>
        ...
        <!-- Toolbar desktop -->
        <div class="wpwc-toolbar d-none d-lg-flex justify-content-center mb-5" role="tablist" aria-label="Filtra per attività">
          <?php echo $chips_html; ?>
        </div>
      <?php endif; ?>
                  ...
                  <?php foreach ($by[$d] as $ev):
                    ...
                    <div class="wpwc-event mb-4" data-cat="<?php echo esc_attr($ev->category_slug ?: ''); ?>">
                      ...
                      <p class="meta paragraph text-sm text-nero text-capitalize">
                        ...
                        <?php
                        // ⛔ Special case: per la categoria 'assemblee' NON mostrare link + testo
                        if (!empty($ev->category_name) && !$is_attivita_ctx && ($ev->category_slug ?? '') !== 'assemblee'): ?>
