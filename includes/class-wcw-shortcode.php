...
class WCW_Shortcode {
  ...
  public static function init(){
    ...
    add_shortcode('weekly_calendar', [__CLASS__, 'render']); // alias comodo
  ...
  public static function render($atts){
    ...
    <div class="wpwc-wrap" id="<?php echo esc_attr($uid); ?>">
      ...
      (function(){
        ...
        function updateURL(slug){ const url = new URL(window.location.href); if (slug) url.searchParams.set('attivita', slug); else url.searchParams.delete('attivita'); window.history.replaceState({}, '', url); }
  ...
  // SOLO giorni con eventi; colore ACF su card
        ...
        <?php foreach ($active_days as $d): ?>
          <div class="wpwc-cell" data-day="<?php echo (int)$d; ?>">
            <?php foreach ($by[$d] as $ev):
              ...
              $bg    = (strlen($color)===7) ? $color.'1A' : '#0000000D'; // 10% alpha
            ?>
