...
class WCW_DB {
  ...
  /** Categorie dal CPT 'attivita' + meta ACF 'colore' (per select admin) */
  ...
  /** Solo attività con almeno un evento (chip frontend) */
  ...
  /** CRUD **/
  public static function insert_event( $name, $weekday, $time, $category_id, $subtitle = '', $time_end = null ) {
    global $wpdb;
    return (bool) $wpdb->insert( self::table_events(), [
      ...
      'time_end'    => ( $time_end !== '' && $time_end !== null ) ? preg_replace( '/[^0-9:]/', '', $time_end ) : null,
      'category_id' => $category_id ? (int) $category_id : null,
    ] );
  }

  public static function update_event( $id, $name, $weekday, $time, $category_id, $subtitle = '', $time_end = null ) {
  ...
  /** Migrazione schema: aggiunge le colonne se mancano */
  public static function maybe_update_schema() {
    global $wpdb; $t = self::table_events();

    $col_subtitle = $wpdb->get_var( $wpdb->prepare( "SHOW COLUMNS FROM $t LIKE %s", 'subtitle' ) );
    if ( ! $col_subtitle ) {
      $wpdb->query( "ALTER TABLE $t ADD COLUMN subtitle VARCHAR(200) NULL AFTER name" );
    }

    $col_time_end = $wpdb->get_var( $wpdb->prepare( "SHOW COLUMNS FROM $t LIKE %s", 'time_end' ) );
    if ( ! $col_time_end ) {
  ...
  public static function delete_event( $id ) {
    global $wpdb;
    return (bool) $wpdb->delete( self::table_events(), [ 'id' => (int) $id ] );
  }
}
endif;
