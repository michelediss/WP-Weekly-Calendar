<?php
// =================================================
// File: includes/class-wcw-db.php
// =================================================
...
class WCW_DB {
  ...
  // ------------------------------------------------
  // CATEGORIE dal CPT 'attivita' con colore ACF 'colore'
  // ------------------------------------------------

  // Tutte le categorie pubblicate (per admin select)
  ...
  // Solo le categorie che hanno almeno un evento (per i filtri frontend)
  public static function get_filter_categories(){
    ...
    LEFT JOIN $e  ev ON (ev.category_id = p.ID)
        WHERE p.post_type=%s AND p.post_status='publish'
     GROUP BY p.ID, p.post_title, p.post_name, c.meta_value
